...
- Number of hidden layers: the classifier head has `1` hidden linear layer.
- Units in each hidden layer: the hidden layer has `1024` units.
- Number of parameters: the full model has `1,524,006` parameters.
- Activations used: the backbone uses `ReLU`, `Hardswish`, and `Hardsigmoid`, and the classifier head uses `Hardswish`.
...
- Training time: about `165.03` seconds total, or about `2.75` minutes.
- Training curve: included as `training_curve.png`, showing epoch number versus empirical risk.
...
I included the training curve generated by my script in `artifacts_pretrained_probe/training_curve.png`. In summary, the final training configuration used pretrained MobileNetV3-Small features, AdamW optimization, cross-entropy loss with label smoothing, standard image normalization, moderate data augmentation, and six-view test-time augmentation at inference, and the final submission pipeline achieved a validation accuracy of `73.33%`.
